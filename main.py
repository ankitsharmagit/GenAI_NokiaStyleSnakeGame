...
class Food:
    ...
    def draw(self, surface):
        """Draws the food with a faster, smaller, size-based pulse."""
        self.pulse_timer += 1
        
        # 1. A sine wave creates a smooth back-and-forth value between 0 and 1.
        #    The speed is controlled by the multiplier (0.5).
        pulse = (math.sin(self.pulse_timer * 0.5) + 1) / 2 

        # 2. A base size and a pulse range are defined.
        base_size = GRID_SIZE - 6 
        pulse_range = 4           
        
        # 3. The current size is calculated by adding the pulse value to the base size.
        current_size = base_size + (pulse * pulse_range)
        
        # 4. An offset is calculated to keep the food centered as it changes size.
        pos_offset = (GRID_SIZE - current_size) / 2
        
        r = pygame.Rect(
            (self.position[0] * GRID_SIZE + pos_offset, self.position[1] * GRID_SIZE + 80 + pos_offset),
            (current_size, current_size)
        )
        # 5. The rectangle is drawn with its new size for the current frame.
